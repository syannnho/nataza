// api/index.js — lapakID Backend v2.2.2
...
function parts(url) {
  // Strip /api prefix in all forms then split
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // ── reports (live chat) ───────────────────────────────────────────────
    if (r0 === 'reports') {
      // POST /api/reports — kirim pesan baru (1x per IP per hari)
      if (!r1 && M==='POST') {
        const ip = getIP(req);
        const b  = await readBody(req);
      ...
      // GET /api/reports/reply-check?name=xxx — cek balasan admin
      ...
      // GET /api/reports — admin lihat semua
      if (!r1 && M==='GET') {
        if (!isAdmin(req)) return fail(res,401,'Unauthorized');
      ...
      // PUT /api/reports/:id/reply — admin balas
      ...
      // PUT /api/reports/:id/read — tandai dibaca
      if (r1 && r2==='read' && M==='PUT') {
        if (!isAdmin(req)) return fail(res,401,'Unauthorized');
        let oid; try{oid=new ObjectId(r1);}catch{return fail(res,400,'ID tidak valid');}
