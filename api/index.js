...
function parts(url) { return url.split('?')[0].replace(/^\/api\/?/,'').split('/').filter(Boolean); }
...
async function ensureSettings(db) {
  ...
  if (!(await col.findOne({key:'music'}))) {
    await col.insertOne({key:'music',value:[]});
  }
}
  ...
  try {
    ...
    if (r0==='notifications') {
      if (!r1 && M==='GET') {
        ...
        const notifs = await db.collection('notifications').find().sort({createdAt:-1}).limit(limit).toArray();
        const unread = await db.collection('notifications').countDocuments({read:false});
    ...
    if (r0==='payment' && !r1 && M==='POST') {
      ...
      await db.collection('notifications').insertOne({
        type:'payment',
        title:'Pesanan Baru',
        message:`${buyer} memesan ID ${idNumber} (${idDoc.tier.toUpperCase()}) — ${finalPrice.toLocaleString('id-ID')}`,
        idNumber:String(idNumber),
        buyer,
        phone,
        finalPrice,
        tier:idDoc.tier,
        paymentId:ins.insertedId,
        ip,
        read:false,
        createdAt:new Date(),
      });
    ...
    if (r0==='payments') {
      ...
      if (r1 && r2==='confirm' && M==='PUT') {
        ...
        await db.collection('notifications').insertOne({
          ...
          message:`Pembayaran ID ${p.idNumber} oleh ${p.buyer} telah dikonfirmasi`,
          idNumber:p.idNumber,
          buyer:p.buyer,
          phone:p.phone,
